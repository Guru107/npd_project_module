...
def validate_task_cancellation(task_name, part_number, subject):
	...
	try:
		...
		frappe.log_error(f"Error validating task cancellation: {str(e)}", "Task Cancellation Validation Error")
	...
	try:
		...
		for dependency in task_doc.depends_on:
			...
			try:
				...
				if (dep_task_doc.part_number == part_number and
					dep_task_doc.iteration_number == iteration_number):
		...
		frappe.log_error(f"Error validating task dependencies: {str(e)}", "Task Validation Error")
	...
	try:
		...
		if cancelled_count > 0:
			return {
				...
				"message": _("Cancelled {0} subsequent task(s) in the sequence for Part {1} (Iteration {2})").format(
					cancelled_count, item_name, iteration_number
				)
			}
			...
			if task_doc.depends_on and len(task_doc.depends_on) > 0:
				...
				for dependency in task_doc.depends_on:
					...
					try:
						...
						if (task_doc.part_number and dep_task_doc.part_number and
							task_doc.part_number == dep_task_doc.part_number and
							task_doc.iteration_number is not None and
							dep_task_doc.iteration_number is not None and
							task_doc.iteration_number == dep_task_doc.iteration_number):
