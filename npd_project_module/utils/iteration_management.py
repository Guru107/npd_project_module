...
def validate_iteration_limit(project_name, part_number):
	...
	if latest_iteration >= 9:  # 0-9 = 10 iterations, so 9 is the last allowed
		return {
			...
			"message": _("Part {0} has reached the maximum iteration limit of 10. Cannot create more iterations.").format(part_number)
		}
...
def get_incomplete_tasks_count(project_name, part_number, iteration_number):
	...
	incomplete_count = frappe.db.count("Task", {
		"project": project_name,
		"part_number": part_number,
...
def get_latest_iteration_number(project_name, part_number):
	...
	""", (project_name, part_number), as_dict=True)
...
def create_new_iteration(project_name, part_number):
	...
	latest_iteration = get_latest_iteration_number(project_name, part_number)
	if latest_iteration is None:
		frappe.throw(_("No previous iteration found. Initial iteration (0) should be created automatically when parts are added to the project."))
	...
	if not cancelled_task:
		frappe.throw(_("No cancelled task found in the latest iteration. Cannot create new iteration without a cancelled task."))
	
	# Check for incomplete tasks in previous iteration
	incomplete_count = get_incomplete_tasks_count(project_name, part_number, latest_iteration)
	...
	if tasks_obsoleted > 0:
		message += _(" Marked {0} incomplete task(s) from previous iteration as obsolete.").format(tasks_obsoleted)
...
def mark_tasks_as_obsolete(project_name, part_number, iteration_number):
	...
	for task in incomplete_tasks:
		try:
			...
			frappe.log_error(f"Error marking task {task.name} as obsolete: {str(e)}")
