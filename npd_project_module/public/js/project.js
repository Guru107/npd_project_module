	...
	after_save: function(frm) {
		...
		const part_numbers_data = frm.doc.part_numbers
			.map(row => {
				...
				const part_number = typeof row.part_number === 'string' ? row.part_number : 
				...
				return {
					...
					iteration_number: (typeof row.iteration_number === 'number') ? row.iteration_number : 
	...
	refresh: function(frm) {
		...
		try {
			...
			if (part_numbers) {
				...
				if (Array.isArray(part_numbers) && part_numbers.length > 0) {
					if (typeof frm.add_custom_button === 'function') {
						frm.add_custom_button(__('Create New Iteration'), function() {
			...
			if (frappe.boot.developer_mode) {
				frappe.msgprint(__('An error occurred in the Project refresh hook. Check console for details.'), 'red');
			}
		}
	}
});
...
function show_iteration_dialog(frm) {
	...
	const part_numbers = parts.map(p => {
	...
	const dialog = new frappe.ui.Dialog({
		...
		fields: [
			...
			},
			{
				...
				options: '<div id="iteration-info-placeholder"><p class="text-muted">' + __('Select a part number to view iteration information.') + '</p></div>'
			},
			{
				fieldtype: 'Section Break',
...
function update_iteration_info(project_name, part_number, dialog) {
	if (dialog.fields_dict.iteration_info && dialog.fields_dict.iteration_info.$wrapper) {
		dialog.fields_dict.iteration_info.$wrapper.html('<p class="text-muted">' + __('Loading iteration information...') + '</p>');
	}
		...
		callback: function(r) {
			...
			if (r.message) {
				...
				frappe.db.get_value('Item', part_number, 'item_name', function(item_r) {
					const item_name = (item_r && item_r.message && item_r.message.item_name) || part_number;
					...
					if (info.latest_iteration !== null && info.latest_iteration !== undefined) {
						infoHTML += `<p><strong>${__('Latest Iteration')}:</strong> ${info.latest_iteration}</p>`;

						if (info.cancelled_task) {
							infoHTML += `<p><strong>${__('Cancelled Task')}:</strong> ${info.cancelled_task.task_subject || info.cancelled_task.task_name}</p>`;
						} else {
							infoHTML += `<p class="text-warning">${__('No cancelled task found in latest iteration. Cannot create new iteration.')}</p>`;
						}
						...
						if (info.new_iteration_start_task) {
							infoHTML += `<p><strong>${__('New Iteration Will Start From')}:</strong> ${info.new_iteration_start_task.task_subject || info.new_iteration_start_task.task_name}</p>`;
							infoHTML += `<p class="text-info"><em>${__('Note: RFQ Data (1st stage) is always completed and will not be regenerated.')}</em></p>`;
						}

						if (info.incomplete_tasks_count > 0) {
							infoHTML += `<p class="text-warning"><strong>${__('Warning')}:</strong> ${info.incomplete_tasks_count} ${__('incomplete task(s) in previous iteration will be marked as obsolete.')}</p>`;
						}
					} else {
					...
					if (!info.can_create_new) {
						infoHTML += `<p class="text-danger"><strong>${__('Error')}:</strong> ${__('Maximum iteration limit (10) reached. Cannot create new iteration.')}</p>`;
					}
					...
					if (info.incomplete_tasks_count > 0) {
						warningHTML = `<div class="alert alert-warning">
							<strong>${__('Warning')}:</strong> ${__('The previous iteration has {0} incomplete task(s). These will be marked as obsolete when you create the new iteration.', [info.incomplete_tasks_count])}
						</div>`;
					}
					...
					if (dialog.fields_dict.warning_message && dialog.fields_dict.warning_message.$wrapper) {
