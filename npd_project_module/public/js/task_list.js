...
frappe.listview_settings["Task"] = {
		...
		if (!listview._npd_menu_items_added) {
			...
			listview.page.add_menu_item(__("Show Cancelled Tasks"), function() {
				...
				const new_filters = filters.filter(function(f) {
					...
					if (Array.isArray(f) && f.length >= 3) {
						...
						return !(f[field_idx] === "status" && f[op_idx] === "!=" && f[val_idx] === "Cancelled");
					}
					return true;
				});
			...
			listview.page.add_menu_item(__("Hide Cancelled Tasks"), function() {
				...
				if (!has_cancelled_filter) {
					...
					const filter_format = filters.length > 0 && filters[0].length === 4 ?
			...
			listview.page.add_menu_item(__("All Open Tasks"), function() {
				...
				listview.filter_area.clear(false).then(function() {
					return listview.filter_area.set([["Task", "status", "in", ["Open", "Overdue"]]]);
				}).then(function() {
					listview.refresh();
				});
			});
	...
	get_indicator: function(doc) {
		...
		if (original_settings.get_indicator) {
			...
			if (doc.iteration_number !== undefined && doc.iteration_number !== null && doc.iteration_number > 0) {
				// Add iteration number to the indicator label
				return [indicator[0] + ` (Iteration ${doc.iteration_number})`, indicator[1], indicator[2]];
			}
			return indicator;
		}
	...
	before_render: function() {
		...
		if (listview && listview.data && Array.isArray(listview.data) &&
			listview.data.length > 0 && !listview._blocked_status_checked) {

			const tasks_with_deps = listview.data.filter(doc =>
				doc && doc.depends_on_tasks && doc.depends_on_tasks.length > 0 &&
				doc.status !== "Completed" && doc.status !== "Cancelled" &&
				doc.part_number && doc.iteration_number !== undefined
			);
	...
	formatters: {
		...
		subject: function(value, doc) {
			...
			if (doc._is_blocked) {
				return value + ' <span class="text-danger" title="Blocked: Dependencies not met">🔒</span>';
			}
			if (doc.depends_on_tasks && doc.depends_on_tasks.length > 0 &&
				doc.status !== "Completed" && doc.status !== "Cancelled") {
		...
		status: function(value, doc) {
			...
			if (doc._is_blocked) {
				return value + ' <span class="text-danger" title="Blocked: Dependencies not met">⚠️</span>';
			}
			return value;
		}
	},
	...
	gantt_custom_popup_html: original_settings.gantt_custom_popup_html || function(ganttobj, task) {
		let html = `
